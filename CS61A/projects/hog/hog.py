...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1):
        current = 0
        if who == 1:
            current = score1 - last_score
        else:
            current = score0 - last_score
        if current > running_high:
            print(current, "point(s)! That's the biggest gain yet for Player", who)
            return announce_highest(who, last_score+current, current)
        else:
            return announce_highest(who, last_score+current, running_high)
    return say
    # END PROBLEM 7
...
def swap_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    if is_swap(score1, opponent_score):
        if opponent_score > score1:
            return 0
    if free_bacon(opponent_score) >= cutoff:
        if is_swap(score1, opponent_score) and score1 > opponent_score:
            return num_rolls
        else:
            return 0
    return num_rolls  # Replace this statement
